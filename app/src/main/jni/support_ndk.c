 ...
 * LongのFieldIdを読み込む
 */
jfieldID ndk_loadLongField(JNIEnv *env, jclass clazz, const char* field_name) {
    assert(clazz != NULL);
    jfieldID result = (*env)->GetFieldID(env, clazz, field_name, "J");
    assert(result != NULL);
    return result;
}
 ...
 * StringのFieldIdを読み込む
 */
jfieldID ndk_loadStringField(JNIEnv *env, jclass clazz, const char* field_name) {
    assert(clazz != NULL);
    jfieldID result = (*env)->GetFieldID(env, clazz, field_name, "Ljava/lang/String;");
    assert(result != NULL);
    return result;
